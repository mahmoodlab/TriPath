...
def get_contour_check_fn(contour_fn='four_pt_easy',
                         patch_size=96,
                         step_size=96,
    ...
    if not use_center_shift:
        ...
        overlap = 1 - float(step_size / patch_size)
        if overlap < 0.25:
            center_shift = 0.375
    ...
    if contour_fn == 'four_pt_hard':
        cont_check_fn = isInContourV3_Hard(patch_size=patch_size, center_shift=center_shift)
    elif contour_fn == 'four_pt_easy':
        cont_check_fn = isInContourV3_Easy(patch_size=patch_size, center_shift=0.5)
    elif contour_fn == 'any_overlap':
        cont_check_fn = isInContourV4(patch_size=patch_size, center_shift=0.5)
    elif contour_fn == 'center':
        cont_check_fn = isInContourV2(patch_size=patch_size)
    elif contour_fn == 'basic':
        cont_check_fn = isInContourV1()
    elif contour_fn == 'all':
        cont_check_fn = lambda x, y : True
    else:
...
class Contour_Checking_fn(object):
    ...
    def __call__(self, cont, pt):
        raise NotImplementedError


class isInContourV1(Contour_Checking_fn):
    def __init__(self, patch_size, center_shift=None):
...
class isInContourV2(Contour_Checking_fn):
    ...
    def __call__(self, cont, pt):
        return 1 if cv2.pointPolygonTest(cont, (pt[0]+self.patch_size//2, pt[1]+self.patch_size//2), False) >= 0 else 0
...
class isInContourV3_Easy(Contour_Checking_fn):
    def __init__(self, patch_size, center_shift=0.5):
        self.patch_size = patch_size
...
# Checks whether the patch contains any of the contour, i.e. they have nonzero overlap
class isInContourV4(Contour_Checking_fn):
    def __init__(self, patch_size, center_shift=0.5):
        self.patch_size = patch_size
        self.shift = int(patch_size*center_shift)
        print(f'patch_size: {patch_size}, center_shift: {center_shift}, shift: {self.shift}')
    ...
    def __call__(self, cont, pt):
        ...
        # First we check if the bounding vertices lie within the contour
        for points in all_points:
            if cv2.pointPolygonTest(cont, points, False) >= 0:
                return 1
        # print(f'Type of c: {type(cont[0][0])}')
        # print(np.array([[4, 5]], dtype=np.float32))
        bounding_sq = np.array([np.array([[x, y]], dtype=np.float32) for (x, y) in all_points])
        # print(bounding_sq)
        # Then we check if any points of the contour lie within the bounding vertices
        # print(cont[0:4])
        # cont_conv = np.array(point[0] for point in cont)
        cont_conv = np.float32(cont)
        # print(cont_conv)
        for points in cont_conv:
            # print(points)
            # pt = (points[0], points[1])
            if cv2.pointPolygonTest(bounding_sq, points[0], False) >= 0:
                return 1
...
###########################################
# Adapted from CLAM/wsi_core/wsi_utils.py #
###########################################
